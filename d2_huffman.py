...
"""
Time Complexity:
Building the Tree: ( O(n \log n) ), where ( n ) is the number of unique characters, due to the priority queue operations when inserting nodes.
Encoding: ( O(n) ), where ( n ) is the number of unique characters because we need to traverse the tree to assign codes.
Decoding: ( O(L \cdot h) ), where ( L ) is the length of the encoded string and ( h ) is the height of the tree (up to ( O(n) ) in the worst case).
Space Complexity:
Tree Storage: ( O(n) ), for storing ( n ) nodes in the Huffman tree.
Encoded String Storage: ( O(L) ).
Decoded String Storage: ( O(N) ), where ( N ) is the length of the original input.
 """